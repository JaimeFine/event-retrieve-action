...
def load_csv_metrics(path: Path, method_key: str | None = None) -> dict:
    ...
    for row in rows:
        ...
        if raw_success_flag not in (None, ""):
            goal_reach_values.append(float(raw_success_flag))
            continue
        final_distance = row.get("final_distance")
        goal_reach_values.append(1.0 if final_distance not in (None, "") and float(final_distance) < 2.0 else 0.0)
    return {
        "success_rate": _mean(float(row["success_rate"]) for row in rows),
        "goal_reach_rate": _mean(goal_reach_values),
        "collision_rate": _mean(float(row["collision_rate"]) for row in rows),
        "warning_rate": _mean(float(row["warning_rate"]) for row in rows),
        "avg_effective_steps": _mean(float(row["effective_steps"]) for row in rows),
        "avg_reaction_time_ms": _mean(float(row["avg_reaction_time_ms"]) for row in rows),
        "avg_final_distance": _mean(float(row["final_distance"]) for row in rows),
    }
        ...
        rows.append(
            {
                ...
                "avg_steps": metrics["avg_effective_steps"],
                "avg_final_distance": metrics["avg_final_distance"],
                "reaction_time_ms": metrics["avg_reaction_time_ms"],
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build a table-ready CSV for the unified metrics table.")
    parser.add_argument(
        "--output-csv",
        default=str(OUTPUTS_DIR / "metrics_regen" / "unified_metrics_table.csv"),
    )
...
def main() -> None:
    ...
    print(f"[build_metrics_table_csv] rows={len(rows)}")
    print(f"[build_metrics_table_csv] output={Path(args.output_csv).expanduser().resolve()}")
