...
DEFAULT_TRAJECTORY_CSV = {
    "medium": ROOT / "outputs" / "metrics_regen" / "all_methods" / "all_methods_medium_seed25_e10_s2000_trajectory_points.csv",
    "extreme": ROOT / "outputs" / "metrics_regen" / "all_methods" / "all_methods_extreme_seed25_e10_s2000_trajectory_points.csv",
}
DEFAULT_OUTPUT_PDF = ROOT / "boxplots.pdf"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build multi-method boxplots from trajectory-point outputs."
    )
    parser.add_argument("--difficulty", choices=["medium", "extreme"], default="extreme")
    parser.add_argument(
        "--trajectory-csv",
        default=None,
        help="Optional override for the trajectory-points CSV.",
    )
    parser.add_argument(
        "--success-only",
        choices=["true", "false"],
        default="true",
        help="Use only goal-reaching episodes so efficiency metrics remain interpretable.",
    )
    parser.add_argument(
        "--output-pdf",
...
def compute_episode_metrics(rows: list[dict]) -> dict | None:
    ...
    last = rows[-1]
    goal = np.asarray(
        [float(last["goal_x"]), float(last["goal_y"]), float(last["goal_z"])],
        dtype=float,
    )
...
def load_metrics(path: Path, success_only: bool) -> tuple[dict[str, list[dict]], dict[str, int]]:
    ...
    metrics_by_method: dict[str, list[dict]] = {method: [] for method in METHOD_ORDER}
    counts_by_method: dict[str, int] = {method: 0 for method in METHOD_ORDER}
...
def draw_boxplot(ax, metric_values: dict[str, np.ndarray], ylabel: str, title: str, counts_by_method: dict[str, int]) -> None:
    ...
    if not methods:
        raise RuntimeError(f"No methods have data for metric {title!r}")
...
def build_figure(metrics_by_method: dict[str, list[dict]], counts_by_method: dict[str, int], difficulty: str, success_only: bool, output_pdf: Path) -> Path:
    metric_values = {
    ...
    draw_boxplot(
        axes[0],
        metric_values["goal_projection_efficiency"],
        ylabel="Projection efficiency",
        title="(a) Goal-Projection Efficiency",
        counts_by_method=counts_by_method,
    )
    draw_boxplot(
        axes[1],
        metric_values["excess_path_ratio"],
        ylabel="Excess path ratio (%)",
        title="(b) Excess Path Ratio",
        counts_by_method=counts_by_method,
    )
    draw_boxplot(
        axes[2],
        metric_values["mean_jerk"],
        ylabel="Mean jerk magnitude",
        title="(c) Mean Jerk Magnitude",
        counts_by_method=counts_by_method,
    )
    draw_boxplot(
        axes[3],
        metric_values["mean_abs_curvature"],
        ylabel="Mean absolute curvature",
        title="(d) Mean Absolute Curvature",
        counts_by_method=counts_by_method,
    )

    suffix = "successful episodes only" if success_only else "all valid episodes"
    fig.suptitle(
        f"Trajectory-quality metrics across five methods ({difficulty} difficulty, {suffix})",
        fontsize=18,
        fontweight="bold",
    )
...
def main() -> None:
    ...
    trajectory_csv = (
        Path(args.trajectory_csv).expanduser().resolve()
        if args.trajectory_csv
        else DEFAULT_TRAJECTORY_CSV[difficulty]
    )
    output_pdf = Path(args.output_pdf).expanduser().resolve()
    success_only = args.success_only == "true"

    metrics_by_method, counts_by_method = load_metrics(trajectory_csv, success_only=success_only)
    ...
    print(f"[plot_boxplots_from_outputs] difficulty={difficulty}")
    print(f"[plot_boxplots_from_outputs] success_only={success_only}")
    print(f"[plot_boxplots_from_outputs] source={trajectory_csv}")
    print(f"[plot_boxplots_from_outputs] output={output_path}")
