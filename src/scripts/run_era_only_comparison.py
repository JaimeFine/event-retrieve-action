...
def install_adapter(sim, adapter: EraAdapter):
...
def evaluate_era_only(
    ...
    for episode_index in range(episodes):
        ...
        trajectory_payload = build_era_trajectory_payload(
            seed=seed,
        ...
        trajectory_rows.extend(flatten_era_trajectory_rows(trajectory_payload))
        rows.append(
            {
                "seed": int(seed),
    ...
    return metrics, rows, trajectory_payloads, trajectory_rows
...
def parse_args():
    parser = argparse.ArgumentParser(description="ERA-only online evaluation with per-episode CSV export.")
    parser.add_argument("--episodes", type=int, default=10)
    parser.add_argument("--steps", type=int, default=2000)
    parser.add_argument("--seed", type=int, default=seeds)
    parser.add_argument("--headless", choices=["true", "false"], default="true")
    parser.add_argument("--finetuned", default="agent_finetuned.pt")
    parser.add_argument("--bank", default="knowledge_bank_snapshot.pt")
...
def default_stem(args) -> str:
    return f"formal_{args.difficulty_level}_seed{args.seed}_e{args.episodes}_s{args.steps}"
...
def main() -> None:
    ...
    adapter = EraAdapter(selector_fn=sim.agent.select_action, device_name=args.device)
    summary, rows, trajectory_payloads, trajectory_rows = evaluate_era_only(
        sim,
        adapter=adapter,
    ...
    print(f"[run_era_only_comparison] json={json_path}")
    print(f"[run_era_only_comparison] csv={csv_path}")
    if args.save_trajectories == "true":
        print(f"[run_era_only_comparison] trajectory_json={trajectory_json_path}")
        print(f"[run_era_only_comparison] trajectory_csv={trajectory_csv_path}")
